...
- The files to watch are `R/` & `tests/` (output of `sisyphus::sisyphus_get_r_and_tests()`)

- The ignored files are the one from `_snaps` (output of `sisyphus::sisyphus_get_testthat_snaps()`), as snaps are modified every time a test is run, creating an infinite loop if ever you watch them.
